            ...
            "embeds": [
                {
                    ...
                    "author": {
                        "name": (
                            "Server"
                            if embed.get("name") == server_console_name
                            else embed.get("name")
                        ),
                        "icon_url": (
                            default_server_console_pic
                            if embed.get("name") == server_console_name
                            else f"{player_icon_api}{embed.get("name")}"
                        ),
                    },
                    "fields": [],
                }
...
def MessageCreation(text: str):
    ...
    if (
        len(match)
        and server_start_message is not None
        and server_start_message != ""
        and status != "online"
    ):
        status = "online"
        player_count = 0
    ...
    if (
        len(match)
        and server_stop_message is not None
        and server_stop_message != ""
